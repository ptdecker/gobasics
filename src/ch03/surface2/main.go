// Surface2 computes an SVG rendering of a 3-D surface function.  It also includes guard code to
// ignore any polygons whose corner values are infinte or NaN.  This version outputs integers instead
// of floats resulting in a lower quality render but nearly three times as fast.  Based upon profiling,
// significant time is spent in fprintf() converting floating point to strings.
...
func corner(i, j int) (int, int, bool) {
...
func genSVG() {
    ...
    for i := 0; i < cells; i++ {
        for j := 0; j < cells; j++ {
            ...
            fmt.Fprintf(out, "<polygon points='%d,%d %d,%d %d,%d %d,%d'/>\n",
